...
% Command to plot right-aligned figure
% Parameters: #1=image name, #2=caption 
\newcommand{\includefiguresmall}[2]
{
    \begin{figure}[h]
    \caption{#2}
    \centering
...
\begin{frame}{\subsectiontitle}
    \begin{figure}[ht]
        ...
        \item It consists of a set of statements (axioms) each of which is true in the situation the ontology is applied to. \citet{source}
        \item DL and semantics... find a nice quote in literature to summarize gently
        \item Classes, Individuals and Relations
    		\end{itemize}
        \end{minipage}
        \hspace{0.5cm}
     \begin{minipage}[b]{0.4\linewidth}
            	\caption{The pizza ontology, visualized as a graph (add source)}
                \includegraphics[width=\textwidth]{pizza-ontology.png}
        \end{minipage}
    \end{figure}
\end{frame}
...
\subsection{\subsectiontitle}
\begin{frame}{\subsectiontitle}
... ODK for automation, standardisation, interoperability... is it convenient for enm?
\end{frame}
